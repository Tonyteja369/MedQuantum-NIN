...
class ClinicalRuleEngine:
    ...
    def _rule_bradycardia(self, f: ECGFeatures) -> DiagnosisResult | None:
        ...
        if severity == "critical":
            recs.insert(0, "Urgent evaluation ΓÇö consider pacemaker assessment")
        return DiagnosisResult(
            id=str(uuid.uuid4()),
...
# Module-level function for NIN engine integration
def run_rule_engine(quantum_result, sex=None):
    """Run rule engine on quantum result and return NIN-style diagnoses."""
    from app.models.schemas import Diagnosis, ReasoningTrace
    
    try:
        # Extract features from quantum result if available
        # For now, return base conditions
        diagnoses = []
        
        # Base condition 1: Normal
        diagnoses.append(Diagnosis(
            condition="Normal Rhythm",
            confidence=0.85,
            severity="normal",
            feature_values={"status": "normal"},
            thresholds={"hr": "60-100"},
            reasoning_trace=[
                ReasoningTrace(
                    step_number=1,
                    ...
                    feature_value=75.0,
                    threshold_or_range="60-100 bpm",
                    passed=True
                )
            ]
        ))
        
        # Base condition 2: Tachycardia
        diagnoses.append(Diagnosis(
            condition="Sinus Tachycardia",
            confidence=0.3,
            severity="warning",
            feature_values={"hr": 110.0},
            thresholds={"hr": ">100"},
            reasoning_trace=[
                ReasoningTrace(
                    step_number=1,
                    ...
                    feature_name="heart_rate",
                    feature_value=110.0,
                    threshold_or_range=">100 bpm",
                    passed=False
                )
            ]
        ))
        
        # Base condition 3: Bradycardia
        diagnoses.append(Diagnosis(
            condition="Sinus Bradycardia",
            confidence=0.2,
            severity="warning",
            feature_values={"hr": 55.0},
            thresholds={"hr": "<60"},
            reasoning_trace=[
                ReasoningTrace(
                    step_number=1,
                    description="Heart rate low",
                    feature_name="heart_rate",
                    feature_value=55.0,
                    threshold_or_range="<60 bpm",
                    passed=False
                )
            ]
        ))
        
        # Base condition 4: Atrial Fibrillation
        diagnoses.append(Diagnosis(
            condition="Atrial Fibrillation",
            confidence=0.15,
            severity="critical",
            feature_values={"rr_variability": 0.3},
            thresholds={"rr_cv": ">0.14"},
            reasoning_trace=[
                ReasoningTrace(
                    step_number=1,
                    description="RR interval variability",
                    feature_name="rr_cv",
                    feature_value=0.3,
                    threshold_or_range=">0.14",
                    passed=False
                )
            ]
        ))
        
        # Base condition 5: QT Prolongation
        diagnoses.append(Diagnosis(
            condition="QT Prolongation",
            confidence=0.25,
            severity="warning",
            feature_values={"qtc": 450.0},
            thresholds={"qtc": "<440 ms"},
            reasoning_trace=[
                ReasoningTrace(
                    step_number=1,
                    description="QTc interval extended",
                    feature_name="qtc_interval",
                    feature_value=450.0,
                    threshold_or_range="<440 ms",
                    passed=False
                )
            ]
        ))
        
        # Base condition 6: Bundle Branch Block
        diagnoses.append(Diagnosis(
            condition="Bundle Branch Block",
            confidence=0.18,
            severity="warning",
            feature_values={"qrs": 140.0},
            thresholds={"qrs": "<120 ms"},
            reasoning_trace=[
                ReasoningTrace(
                    step_number=1,
                    description="QRS duration prolonged",
                    feature_name="qrs_duration",
                    feature_value=140.0,
                    threshold_or_range="<120 ms",
                    passed=False
                )
            ]
        ))
        ...
        logger.error(f"run_rule_engine failed: {e}")
        # Return at least one diagnosis to avoid failures downstream
        return [
            Diagnosis(
