...
def apply_explainability(diagnosis):
    """Apply explainability to a diagnosis object and populate reasoning_trace."""
    from app.models.schemas import Diagnosis, ReasoningTrace
    
    try:
        # Ensure diagnosis has reasoning_trace populated
        if not hasattr(diagnosis, 'reasoning_trace') or not diagnosis.reasoning_trace:
            # Create default reasoning traces
            diagnosis.reasoning_trace = [
                ReasoningTrace(
                    step_number=1,
                    description=f"Analyzing {diagnosis.condition} condition",
                    feature_name="diagnosis",
                    feature_value=diagnosis.condition,
                    threshold_or_range="Clinical evaluation",
                    passed=(diagnosis.confidence > 0.5)
                ),
                ReasoningTrace(
                    step_number=2,
                    description=f"Confidence score assessment: {diagnosis.confidence:.2%}",
                    feature_name="confidence",
                    feature_value=diagnosis.confidence,
                    threshold_or_range=">0.5",
                    passed=(diagnosis.confidence > 0.5)
                )
            ]
        
        # Ensure at least the default traces are there
        if len(diagnosis.reasoning_trace) < 2:
            while len(diagnosis.reasoning_trace) < 2:
                diagnosis.reasoning_trace.append(
                    ReasoningTrace(
                        step_number=len(diagnosis.reasoning_trace) + 1,
                        description=f"Supporting evidence for {diagnosis.condition}",
                        feature_name="evidence",
                        feature_value=diagnosis.severity,
                        threshold_or_range="Clinical assessment",
                        passed=(diagnosis.confidence > 0.5)
                    )
                )
        
        logger.info(f"Explainability applied to {diagnosis.condition}: {len(diagnosis.reasoning_trace)} traces")
        return diagnosis
        
    except Exception as e:
        logger.error(f"apply_explainability failed: {e}")
        # Return diagnosis anyway, even if explainability failed
        return diagnosis
